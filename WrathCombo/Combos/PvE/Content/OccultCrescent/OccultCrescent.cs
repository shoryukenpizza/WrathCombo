    ...
    internal static uint BestPhantomAction()
    {
            ...
            CanWeave())
        {
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Bard_MightyMarch, MightyMarch) &&
                !HasStatusEffect(Buffs.MightyMarch) && PlayerHealthPercentageHp() <= Config.Phantom_Bard_MightyMarch_Health)
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Berserker) && HasBattleTarget())
        {
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Berserker_Rage, Rage) &&
                InMeleeRange() && CanWeave())
                return Rage; //buff

            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Berserker_DeadlyBlow, DeadlyBlow) &&
                GetStatusEffectRemainingTime(Buffs.PentupRage) <= 3f && GetTargetDistance() <= 5f && !CanWeave())
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Cannoneer) && !CanWeave())
        {
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Chemist) && !CanWeave())
        {
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Chemist_OccultPotion, OccultPotion) &&
                PlayerHealthPercentageHp() <= Config.Phantom_Chemist_OccultPotion_Health)
                return OccultPotion;

            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Chemist_OccultEther, OccultEther) &&
                LocalPlayer.CurrentMp <= Config.Phantom_Chemist_OccultEther_MP)
                return OccultEther;
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Freelancer) &&
            ...
            PlayerHealthPercentageHp() <= Config.Phantom_Freelancer_Resuscitation_Health && !CanWeave())
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Geomancer) && !CanWeave())
        {
            ...
            if (IsEnabled(CustomComboPreset.Phantom_Geomancer_Weather))
            {
                if (IsEnabledAndUsable(CustomComboPreset.Phantom_Geomancer_Sunbath, Sunbath) &&
                    PlayerHealthPercentageHp() <= Config.Phantom_Geomancer_Sunbath_Health)
                    return Sunbath; //heal
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Knight) && !CanWeave())
        {
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Knight_PhantomGuard, PhantomGuard) &&
                PlayerHealthPercentageHp() <= Config.Phantom_Knight_PhantomGuard_Health && InCombat())
                return PhantomGuard; //mit

            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Knight_Pray, Pray) &&
                PlayerHealthPercentageHp() <= Config.Phantom_Knight_Pray_Health && !HasStatusEffect(Buffs.Pray))
                return Pray; //regen

            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Knight_OccultHeal, OccultHeal) &&
                PlayerHealthPercentageHp() <= Config.Phantom_Knight_OccultHeal_Health && LocalPlayer.CurrentMp >= 5000)
                return OccultHeal; //heal

            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Knight_Pledge, Pledge) &&
                PlayerHealthPercentageHp() <= Config.Phantom_Knight_Pledge_Health && InCombat())
                return Pledge; //inv
        }
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Monk))
        {
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Monk_OccultChakra, OccultChakra) &&
                CanWeave() && PlayerHealthPercentageHp() <= Config.Phantom_Monk_OccultChakra_Health)
                return OccultChakra; //heal

            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Monk_Counterstance, Counterstance) &&
                !HasStatusEffect(Buffs.Counterstance) && !CanWeave())
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Monk_OccultCounter, OccultCounter) &&
                CanWeave() && HasStatusEffect(Buffs.Counterstance) && ActionReady(OccultCounter))
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Oracle) && !CanWeave())
        {
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Oracle_Predict, Predict) &&
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Oracle_Blessing, Blessing) &&
                HasStatusEffect(Buffs.PredictionOfBlessing) && PlayerHealthPercentageHp() <= Config.Phantom_Oracle_Blessing_Health)
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Oracle_Starfall, Starfall) &&
                HasStatusEffect(Buffs.PredictionOfStarfall) && PlayerHealthPercentageHp() >= Config.Phantom_Oracle_Starfall_Health)
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Ranger) && CanWeave())
        {
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Ranger_PhantomAim, PhantomAim) &&
                !HasStatusEffect(Buffs.PhantomAim, anyOwner: true) &&
                GetTargetHPPercent() >= Config.Phantom_Ranger_PhantomAim_Stop)
                return PhantomAim; //damage buff

            //if (IsEnabledAndUsable(CustomComboPreset.Phantom_Ranger_OccultFalcon, OccultFalcon) && !HasStatusEffect(Buffs.OccultUnicorn)) //TODO: add something for ground targeting? atm needs something like Reaction to work correctly
        ...
        if (IsEnabled(CustomComboPreset.Phantom_Thief))
        {
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_Thief_PilferWeapon, PilferWeapon) &&
                CanWeave() && !HasStatusEffect(Debuffs.WeaponPlifered, CurrentTarget))
        ...
        if (IsEnabled(CustomComboPreset.Phantom_TimeMage) && !CanWeave())
        {
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_TimeMage_OccultMageMasher, OccultMageMasher) &&
                !HasStatusEffect(Debuffs.OccultMageMasher, CurrentTarget))
            ...
            if (IsEnabledAndUsable(CustomComboPreset.Phantom_TimeMage_OccultSlowga, OccultSlowga) &&
                !HasStatusEffect(Debuffs.Slow, CurrentTarget) &&
